...
let liveAudit = null;
try {
  liveAudit = JSON.parse(await readFile(path.join(root, 'seo/live-crawl-audit.json'), 'utf8'));
} catch {}
const liveInHtml = (liveAudit?.sites?.in?.pages || []).filter(page => page.title !== undefined);
const liveAuditDate = liveAudit?.generated_at
  ? new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Asia/Kolkata' }).format(new Date(liveAudit.generated_at))
  : generatedDate;
const liveHome = liveInHtml.find(page => page.requested_url === 'https://www.millionmindstechcity.in/');
const liveAverageMs = liveInHtml.length
  ? Math.round(liveInHtml.reduce((sum, page) => sum + (page.duration_ms || 0), 0) / liveInHtml.length)
  : 0;
...
if (sitemapLocSet.size !== sitemapLocs.length) errors.push('sitemap: duplicate canonical URL');
const expectedSitemapLocs = new Set(records.map(record => record.canonical));
for (const loc of sitemapLocSet) if (!expectedSitemapLocs.has(loc)) errors.push('sitemap: unexpected or non-indexable URL ' + loc);
for (const loc of expectedSitemapLocs) if (!sitemapLocSet.has(loc)) errors.push('sitemap: missing indexable URL ' + loc);

if (routeMap.has('/site-index')) {
  const siteIndexHtml = await readFile(routeMap.get('/site-index'), 'utf8');
  for (const record of records) {
    if (record.route === '/site-index') continue;
    const href = record.route === '/' ? '/' : record.route;
    if (!siteIndexHtml.includes('href="' + href + '"')) errors.push('/site-index: missing link to ' + record.route);
  }
}
...
const refreshedReportHeader = currentReportHeader
  .replace('Generated: 12 September 2026', 'Generated: ' + generatedDate)
  .replace('Scope: canonical static HTML routes before production deployment. HTTP status is the expected static-host response and must be rechecked after deployment.', 'Scope: complete static HTML route validation plus a live crawl of every canonical URL currently listed in the production sitemap.')
  .replace('18 distinct-intent pages were added: one nearby-area hub, six locality guides, six commercial-intent pages and five decision-support articles. The sitemap now contains 58 canonical `.in` URLs.', 'The non-branded content system now includes an Ahmedabad-wide office-space pillar plus distinct location, office-format, GCC, IT-park, retail and decision-guide intents. The sitemap contains ' + records.length + ' canonical `.in` URLs.')
  .replace('Deploy the generated 58-URL site', 'Deploy the generated ' + records.length + '-URL site')
  .replace('Deploy this repo, then verify', 'Verify this deployment, including')
...
const currentQualitySummary = '- **Non-branded keyword gap addressed:** `/office-space-ahmedabad` now owns the broad transactional cluster for office space, office rentals, commercial offices and office leasing in Ahmedabad. Format-, location- and project-specific pages retain narrower primary intents.\n- **Project keyword gap addressed:** unique master-plan, brochure and rent-intent pages remain supported by sourced project and M One guidance. No live rent or vacancy was invented.\n- **Live crawl (' + liveAuditDate + '):** all ' + liveInHtml.length + ' sitemap URLs on `www` returned HTTP 200 with a description, canonical, one H1 and a favicon declaration. The homepage responded in ' + (liveHome?.duration_ms || 0) + ' ms during this crawl; the HTML-page average was ' + liveAverageMs + ' ms.\n- **Open deployment blocker:** direct HTTPS access to the apex domain `millionmindstechcity.in` still fails before it can redirect to `www`; DNS, certificate and hosting-domain attachment must be repaired.\n\n';
