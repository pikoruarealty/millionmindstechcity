...
const refreshedReportHeader = currentReportHeader
  ...
  .replace('Scope: canonical static HTML routes before production deployment. HTTP status is the expected static-host response and must be rechecked after deployment.', 'Scope: static HTML route validation plus live spot checks of the new pages and sitemap; the expected HTTP column is not a complete live crawl.')
...
const currentQualitySummary = '- **Project keyword gap addressed:** added unique master-plan, brochure and rent-intent pages; strengthened the M One floor-plan guide with evidence from the official brochure. No live rent or vacancy was invented.\n- **Live spot check:** the three new canonical paths returned HTTP 200 with the new content, the homepage linked to them, and the live sitemap exposed ' + records.length + ' URLs after deployment. The apex HTTPS host still failed a direct TLS check and needs domain/certificate repair.\n\n';
